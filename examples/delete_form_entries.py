"""Delete one example event form for one athlete in sandbox without re-uploading."""
...
from ams_smartabase import SmartabaseClient, load_credentials, run_example_event_delete


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Count, delete, and recount one example event form in sandbox without uploading replacement data.",
    )
    parser.add_argument(
        "--csv-path",
        default="use_case_examples/synthetic_data/csv/training load template 1777445863459.csv",
        help="Example CSV file used to identify the target athlete/form/date range.",
    )
    parser.add_argument(
        "--base-dir",
    ...
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Run the live delete calls. Omit for dry run.",
    )
    parser.add_argument(
        "--confirm",
        action="store_true",
    ...
    parser.add_argument(
        "--delete-all-in-range",
        action="store_true",
        help="Sandbox-only override: delete all existing events for the form/user/date range.",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.execute and "sandbox" not in credentials.url.lower():
        print("Refusing to execute delete operations because SMARTABASE_URL does not contain 'sandbox'.", file=sys.stderr)
        return 2
    ...
    result = run_example_event_delete(
        client,
